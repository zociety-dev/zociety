#!/usr/bin/env python3
"""nfprov-diff.py -- zociety's `mark-added` front end for the nerd-fonts provenance tool.
...
vendored copy of upstream's reference tool in bin/nfprov.py (with
bin/nfprov-mapping.json), so the output is byte-compatible with what the
fork's P+ fonts render and what `nfprov.py inspect` reports.

    nfprov-diff.py mark-added --base BLOB_OR_FILE [--mode=vs|pua] FILE
    nfprov-diff.py --selftest
...
UPSTREAM_PATH = Path(__file__).resolve().with_name("nfprov.py")


@lru_cache(maxsize=1)
def upstream():
    """Import the vendored canonical bin/nfprov.py as a module (loaded by path
    so the hyphen-free filename is never confused with this front end)."""
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="nfprov-diff.py")
